...
public:
  ...
  vector<FLOAT_T> compute_basis_coordinates(const vector<COMPL_FLOAT> & z) const {
    ...
    //cout << z.size() << " is the size and the tmp is " << sig_R_basis_tmp[1].size() << endl;
    for(size_t i = 0; i < ret.size(); i++){
  ...
  vector<INT_T> encode(const vector<COMPL_FLOAT> & z) const {
      //cout << z.size() << " size and deg " << this->poly_deg() << endl;
    ...
    //cout << pi_inv_z.size() << " is the new z size " << endl;
